document.addEventListener('DOMContentLoaded', function() {
    ...
    function activateLinkAndOpenDetails(targetId) {
        ...
        if (targetDetails) {
            if (!targetDetails.open) targetDetails.open = true;
            setTimeout(()=>{
    ...
    detailsElements.forEach((details)=>{
        ...
        if (summary) summary.addEventListener('click', function(e) {
            setTimeout(()=>{
                ...
                if (details.open) activateLinkAndOpenDetails(detailsId);
                else {
                    const targetLink = document.querySelector(`.manual__tree-link[data-target="${detailsId}"]`);
                    if (targetLink) targetLink.classList.remove('is-active');
                }
            }, 10);
        });
    });
    function handleHashChange() {
        ...
        if (hash && (hash === 'family' || hash === 'inheritance')) activateLinkAndOpenDetails(hash);
        else {
            detailsElements.forEach((details)=>{
                details.open = false;
            });
...
//# sourceMappingURL=DocumentsList.1f4b0d68.js.map
